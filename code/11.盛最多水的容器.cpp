...
public:
    int maxArea_1(vector<int>& height) {
           // n 为height的size，最少为2·   
           // 这个是求面积最大，面积与长宽相关，长a:为x轴的距离，也就是下标(x1,x2)的差值
           // 宽b为对应(x1,x2)的数组元素(2个元素)的最小值
           if(0 == height.size())
           {
               return 0;
           }

           // 初步想法：长a 最大是height.size()-1，最小是1，可以从最大一直缩小到最小
           // 然后对应求出每个a对应的可能的面积的最大值
           // 最后所有a对应面积的再求一个最大值（就是结果了）

            int nMaxArea = 0;
            for(int a = height.size()-1;a>0;a--)
            {
                // a可能对应不止一对的(x1,x2) a = abs(x1-x2) x(min) = 0; x(max)=height.size()-1
                // 寻找a对应的(x1,x2)的下标对
                std::map<int,int> map_x;
                int x1=0,x2=0;
                while (x1 < height.size())
                {
                    x2 = x1+1; // 确保x2大于x1
                    while (x2 < height.size())
                    {
                        if(a == x2-x1)
                        {
                            //找到数对
                            map_x.insert(map_x.end(), {x1,x2});
                            break;
                        }
                        x2++;
                    }
                ...
                // 求map_x 存储的下标对 对应面积
                int curMaxArea = 0;
                for(auto pair_x : map_x)
                {
                    int b = min(height[pair_x.first], height[pair_x.second]);
                    if(a*b > curMaxArea)
                    {
                        curMaxArea = a*b;
                    }
                ...
                // 判断当前的a对应的面积是否大于已存储的面积最大值，是的话更新结果
                if(curMaxArea > nMaxArea)
                {
                    nMaxArea = curMaxArea;
                }
            }
    ...
    int maxArea_2(vector<int>& height) {
           ...
           // 最后所有a对应面积的再求一个最大值（就是结果了）
            int nMaxArea = 0;
            // 获取height的最大值
            vector<int>::iterator max_iter = std::max_element(height.begin(), height.end());
            int max_height = *max_iter;
            for(int a = height.size()-1;a>0;a--)
            {
                // 如果已经保存的最大面积nMaxArea除以最大的高度(上面是宽b的含义)，已经大于长a
                // 那说明最大面积已经找到了，不用在做多余的循环
                if((double)nMaxArea/max_height > a)
                {
                    break;
                }
                ...
                while (x1 < height.size())
                {
                    x2 = height.size()-1; // 确保x2大于x1
                    if(x2 - x1 < a) // 如果x2的最大值-x1 都小于a,那么就不会有符合条件的x1,x2
                    {
                        // 直接跳出循环
                        break;
                    }
    ...
    // 双指针
    // 左右柱子高度为： h1,h2(我们假设h1<h2, 右边柱子高)，左右柱子相距长度 a
    // 此时的 面积为：  min(h1,h2) *a
    // 我们要移动左右柱子；
    // 如果移动高柱子： 得到的 h3 和 相距长度a1  此时的面积为 min(h1,h3)*a1 
    // 我们不移动高柱子，而移动矮柱子，就是要证明：   min(h1,h3)*a1 < min(h1,h2) *a; (移动高柱子的话，面积只会更小)
    // 能确定的是a1< a
    // 现在就是h3 和h2的关系  如果h2<h3; 根据前面的：h1<h2，则 h1<h3,  min(h1,h3) = h1 = min(h1,h2)
    // 					   如果h2>h3 , 则h1和h3关系不明确，若 h1<h3时，同上; 若h1>h3   h3 =  min(h1,h3) < min(h1,h2) = h1				   
    // 所以也就证明了 min(h1,h3)*a1 < min(h1,h2) *a; 
    ...
    int maxArea(vector<int> &height)
    {
        ...
        // 暴力算法，双循环
        // x1 从0开始，x2从height.size() - 1开始
        // 长方形的长a = x2 - x1
        int x1 = 0;
        while (x1 < height.size())
        {
            int x2 = height.size() - 1; // 确保x2大于x1
            while (x1 < x2)
            {
                int a = x2 - x1;
                // 如果已经保存的最大面积nMaxArea除以最大的高度(上面是宽b的含义)，已经大于长a
